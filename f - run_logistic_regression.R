...
run_logistic_regression <- function(long_nhanes_subset,
                                  conversion,
                                  weights_dataset,
                                  chem_master,
                                  nhanes_subset)
{
  ...
  df_regressions_i <- long_nhanes_subset %>%
    filter(chemical_codename %in% c("LBXCOT",
                                    "LBXBCD",
                                    "LBXSCU")) %>%
    filter(celltype_codename %in% c("LBXWBCSI")) %>%
    group_by(celltype_codename, chemical_codename) %>%
    do(run_if_else_glm_logistic_wt(.,
                                   weights_dataset,
                                   chem_master, 
